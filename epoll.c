...
void server_run() { /* test server */
	...
	for(;;) {
		...
		for(i = 0; i < nfds; i++) {
			...
			if (events[i].events & (EPOLLRDHUP | EPOLLHUP)) { /* check if the connection is closing */
